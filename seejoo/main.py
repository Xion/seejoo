...
"""
import logging
from optparse import OptionParser  # Not argparse since we target Python 2.6 as well
import os
import sys
...
def create_data_directory():
    ...
    try:
        ...
        pass  # errors handled below

    if not os.path.exists(data_dir):
        logging.fatal("Could not create data directory: %s", data_dir)
        exit(1)
    if not os.path.isdir(data_dir):
        logging.fatal(
            "Data directory path already exists and is NOT a directory: %s",
            data_dir)
        exit(1)
